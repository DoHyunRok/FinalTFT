...
<body>
	...
	<div id="parapara">
		<form action="BoardSearch.do" method="POST">
			...
			</select> <input type="text" placeholder="검색어를 입력해주세요" id="boardSearch"
				name="boardSearch"> <input type="submit" value="검색"
				id="boardSeach1">
		</form>
	</div>
	<div class="morelist">
		<input type="submit" value="더보기" onclick="morelist()" id="morelist">
		<br>
	</div>
	<div class="freewrite">
		<form action="freewrite.do">
			<input type="submit" value="글쓰기" id="freewrite">
		</form>
	</div>
						...
						$
								.each(
										...
										function(i) {
											str += '<TD>'
													+ result[i].rownum
													+ '</TD><TD colspan="2"><a href="freeview.do?seq=${board.seq}">'
													+ result[i].title
													+ '</a></TD>' + '<TD>'
													+ result[i].writer
													+ '</TD><TD>'
			...
			success : function(data) {
				...
				$.each(csort, function(i) {
					str += '<TD>' + csort[i].rownum
							+ '</TD><TD colspan="2"><a href="freeview.do?seq='
							+ csort[i].seq + '">' + csort[i].title
							+ '</a></TD>' + '<TD>' + csort[i].writer
							+ '</TD><TD>' + csort[i].regdate + '</TD><TD>'
							+ csort[i].cnt + '</TD>'
					str += '</TR>'
				});
				$("#freebbs").append(str);
